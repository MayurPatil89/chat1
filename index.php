...
function postApi($action,$profileID,$AccountID)
{
	...
	if ($action == "version" && isset($profileID) && isset($AccountID)) {
		$plain = "version|".$profileID."|" ."$AccountID";
		$request = encrypts($plain);
		$post = strtr($request, '+/=', '-_,' );
		$curlURL = $domain."?data=".$post;
...
if($method == 'POST'){
	...
	switch ($action) 
	{
		...
		case 'hi':
			$check->speech =  "Hi, Its a worderful day , welcome to the webchat ";
			$check->displayText =  "Hi, Its a worderful day , welcome to the webchat ";
		break;
		...
		default:
			...
			$check->speech ="Hi, Its a worderful day , welcome to the chatbot ";
			$check->displayText =  "Hi, Its a worderful day , welcome to the chatbot ";
		break;
	}
